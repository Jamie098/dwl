...
void scenebuffersetopacity(struct wlr_scene_buffer *buffer, int sx, int sy,
                           void *data) {
  Client *c = data;
  /* xdg-popups are children of Client.scene, we do not have to worry about
     messing with them. */
  wlr_scene_buffer_set_opacity(
      buffer, (c->isfullscreen || c == focustop(selmon)) ? 1 : c->opacity);
}
...
void setfullscreen(Client *c, int fullscreen) {
  ...
  wlr_scene_node_for_each_buffer(&c->scene_surface->node, scenebuffersetopacity,
                                 c);
  arrange(c->mon);
  printstatus();
}
